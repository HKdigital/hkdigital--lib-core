<script>
  ...
  /** @type {Object | null} */
  const currentMainFolder = $derived(
    selectedMainFolder ? data.navigationData[selectedMainFolder] : null
  );

  // Get subfolders for selected main folder
  /** @type {string[]} */
  const subFolders = $derived(
    currentMainFolder ? Object.keys(currentMainFolder.subfolders) : []
  );

  // Get current subfolder data
  /** @type {Object | null} */
  const currentSubFolder = $derived(
    currentMainFolder && selectedSubFolder
      ? currentMainFolder.subfolders[selectedSubFolder]
      : null
  );
  ...
  const examples = $derived(
    currentSubFolder
      ? currentSubFolder.examples
      : currentMainFolder
        ? currentMainFolder.examples
        : []
  );
  ...
  <div class="navigation-columns">
    ...
    {#if selectedMainFolder && (selectedSubFolder || currentMainFolder?.examples.length > 0)}
      <div class="column" data-column="examples">
        <h2 class="type-heading-h2 mb-20up">
          {selectedSubFolder || selectedMainFolder}
        </h2>
        <nav class="folder-list">
